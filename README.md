...
**Distribución de carga SLURM** — granularidad por etapa:

| Etapa | Tipo | Concurrencia | Memoria/tarea | Razón |
|---|---|---|---|---|
| fastp | array 1-12%6 | 6 | 24 GB | I/O ligado |
| hisat2-build | único | — | 64 GB | una sola vez |
| hisat2 align | array 1-12%4 | 4 | 48 GB | RAM por índice cargado |
| stringtie | array 1-12%6 | 6 | 16 GB | ligero |
...
```bash
# Lanzar todo el pipeline (1 comando)
bash pipeline/scripts/master_launcher.sh /LUSTRE/.../microplastics/Manifest.tsv

# Monitoreo
squeue -u $USER -o '%.10i %.20j %.8T %.10M %.6D %R'
sacct  -j <jobid>  --format=JobID,JobName,State,Elapsed,MaxRSS,ReqMem
tail -f pipeline/logs/fastp_*.err
